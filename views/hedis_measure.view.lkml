view: hedis_measure {
  derived_table: {
    sql: select * from "SCH_ALL_HEALTH_CHOICE"."HEDIS_MEASURES"
...
##USE OF SPIROMETRY TESTING IN THE ASSESSMENT AND DIAGNOSIS OF COPD
  ...
  dimension: measure_name{
    type: string
    label: "Measure Name"
    sql: case when ${aap_compliant} is null then 'AAP'
              when ${aba_compliant} is null then 'ABA'
              when ${ccs_compliant} is null then 'CCS'
              when ${cbp_compliant} is null then 'CBP'
              when ${cdc_compliant} is null then 'CDC'
              when ${col_compliant} is null then 'COL'
              when ${bcs_compliant} is null then 'BCS'
              when ${col_compliant} is null then 'COL'
              when ${omw_compliant} is null then 'OMW'
              when ${smd_compliant} is null then 'SMD'
              when ${spr_compliant} is null then 'SPR'
              when ${bcs_compliant} is null then 'BCS'
    else '0'
    end;;
  }
  ...
  dimension: measure_n{
    type: string
    label: "Measure Ne"
    sql: case when ${TABLE}.${aap_compliant} then 'AAP'
          else '0'
          end;;
  }
